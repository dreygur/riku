...
- [x] **`riku quickstart`**: scaffolds a sample app and prints the exact `git remote add` line so a new user can deploy in under five minutes.
- [x] **Better first-deploy output**: `git push` streams the deploy log line-by-line (`DeployLogger`, Heroku-style `-----> ` prefix), logs the detected runtime, and ends with a prominent `<app> deployed!` plus the live URL (from `NGINX_SERVER_NAME`) or a hint to add a domain.

### Phase 1: Finish the Dashboard, **shipped**

- [x] Two dashboards now exist: the embedded single-binary HTML dashboard
...
- [x] Mutating actions (deploy/restart/stop, addon provision/bind/unbind,
...
- [x] **Backups**: `riku backup <app>` / `riku restore <app>` (tar-based; S3/remote-storage output is still open).
- [x] **Rollback**: `riku rollback <app>`, atomic symlink swap under the per-app deploy lock.
- [x] **Zero-downtime deploys**: canary/generation-based health-gated cutover (`riku-supervisor::process::generation`/`orchestration`).
- [x] **`riku doctor`**: diagnoses nginx/systemd/permissions/disk/cert state.

### Phase 3: Stateful Apps, **shipped**

The biggest single unblock for solo devs: shipped as plugins (see Track B), not core bloat.

- [x] **Managed datastores as addons**: Postgres, Redis, SQLite-volume ship
...
- [x] `bind` injects the addon's returned env (e.g. `DATABASE_URL`) into the
...
- [x] **Plugin protocol versioned**: `RIKU_PLUGIN_API=1`, published and
  actively maintained in `PLUGIN_PROTOCOL.md` (verb I/O, capabilities,
  every seam and event listed below).
- [x] **`riku plugins scaffold <name>`**: generates a bundle skeleton
...
- [x] **Checksum + signature verification on install**: a pinned `sha256` mismatch is rejected; an Ed25519 `signature` (`riku plugins keygen`/`sign`) must verify against a key the operator explicitly trusts (`riku plugins trust add/list/remove`) or the install is rejected outright.
- [x] **Pinned versions + lockfile** (`riku-plugins.lock`), records name, source, version, checksum, and verifying key; no silent auto-update of executable code.
- [x] **Explicit trust on `marketplace add`**: third-party marketplaces are opt-in.
- [x] **Capability declaration** (`network`/`writes`/`privileged`), shown on install, enforced at spawn time via Landlock + `no_new_privs` wherever the kernel supports it.
...
- [x] **`riku plugins doctor`**: validates installed plugins against the current API version and re-checks integrity against the lockfile.
...
Honest priority order across both tracks (items marked [x] shipped since this was last written):

1. (shipped) Installer + `quickstart` + first-deploy output, cheap, unblocks *all* adoption. All shipped together in `41bb349`.
2. (shipped) Dashboard, mutating actions included, both embedded and Next.js.
3. (shipped) Plugin contract v1 + `scaffold`.
4. (shipped) Addon seam + Postgres/Redis/SQLite-volume as installable example addons (not bundled by default, an explicit `riku plugins add`, since addons hold credentials).
5. (shipped) Backups + rollback + zero-downtime cutover + `doctor`.
6. (shipped) Marketplace (`riku plugins marketplace add/list/remove`, `search`, `add name@market`) + lockfile + checksum/signature verification + capability declaration, shipped together from the start.
7. (shipped) Notifier / router / filter / custom-event / UI-panel plugins. A searchable docs gallery beyond the existing reference pages is still open.
8. (shipped) Dashboard mutating actions. WASM plugin sandbox and a dashboard auth-*provider* seam (SSO as something a plugin supplies), **still open.**
