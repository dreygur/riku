...
fn generate_nginx_config_from_template(
    ...
    // If Cloudflare ACL is enabled, fetch live Cloudflare IPs
...
pub fn generate_cloudflare_ips_config(paths: &crate::config::RikuPaths) -> Result<()> {
    ...
    // Check if we need to refresh (file older than 24 hours)
    let should_refresh = if cloudflare_conf_path.exists() {
        if let Ok(metadata) = fs::metadata(&cloudflare_conf_path) {
    ...
    config_content.push_str("# Cloudflare IP ranges - auto-generated by Riku\n");
    config_content.push_str("# Do not edit manually - will be refreshed every 24 hours\n\n");

    // Validate that a line looks like an IPv4 or IPv6 CIDR/address before
    // writing it into the nginx config.  This prevents a compromised upstream
    // response from injecting arbitrary nginx directives.
    let is_valid_cidr = |line: &str| -> bool {
        // Accept only characters that appear in valid IP/CIDR notation.
        // Format: digits, dots, colons (IPv6), hex digits, and an optional /prefix.
        // E.g. "103.21.244.0/22" or "2400:cb00::/32".
        let valid_chars = line
            .chars()
            .all(|c| c.is_ascii_hexdigit() || c == '.' || c == ':' || c == '/');
        // Must contain at least one '.' or ':' (IP separator) and no spaces.
        valid_chars && (line.contains('.') || line.contains(':')) && !line.contains(' ')
    };

    // Fetch IPv4
    if let Ok(response) = client.get(ipv4_url).send() {
        if let Ok(ips) = response.text() {
            for ip in ips.lines() {
                let ip = ip.trim();
                if !ip.is_empty() {
                    if is_valid_cidr(ip) {
                        config_content.push_str(&format!("allow {};\n", ip));
                    } else {
                        eprintln!("Warning: skipping invalid Cloudflare IPv4 entry: {:?}", ip);
                    }
                }
            }
        }
    ...
    // Fetch IPv6
    if let Ok(response) = client.get(ipv6_url).send() {
        if let Ok(ips) = response.text() {
            for ip in ips.lines() {
                let ip = ip.trim();
                if !ip.is_empty() {
                    if is_valid_cidr(ip) {
                        ...
                        eprintln!("Warning: skipping invalid Cloudflare IPv6 entry: {:?}", ip);
                    }
                }
            }
        }
    ...
    // Add SSH_CLIENT IP if available
